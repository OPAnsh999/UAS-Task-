...
'''
path = f"D:\\Ansh Tayal\\DTU ECE\\Societies\\UAS\\UAS Task\\2.png"
temp_paths = {"circle":"D:\\Ansh Tayal\\DTU ECE\\Societies\\UAS\\UAS Task\\circle.png","square":"D:\\Ansh Tayal\\DTU ECE\\Societies\\UAS\\UAS Task\\square.png","triangle":"D:\\Ansh Tayal\\DTU ECE\\Societies\\UAS\\UAS Task\\triangle.png","star":"D:\\Ansh Tayal\\DTU ECE\\Societies\\UAS\\UAS Task\\star.png"}


coordinates = wmap(path,temp_paths)
img_rgb = cv.imread("D:\\Ansh Tayal\\DTU ECE\\Societies\\UAS\\UAS Task\\res.png")
...
# Display updated res.png with arrows
cv.imshow("Assignments on res.png", img_rgb)
cv.waitKey(0)
cv.destroyAllWindows()

# Save updated image
cv.imwrite("D:\\Ansh Tayal\\DTU ECE\\Societies\\UAS\\UAS Task\\res_with_arrows.png", img_rgb)

# Initialize score per pad
pad_scores = {pad['name']: 0 for pad in pad_list}

# Calculate score for each pad
for i, casualty in enumerate(casualty_list):
    pad = assignments[i]
    if pad is None:
        continue  # casualty not assigned
    s_score = shape_score[casualty['shape'].lower()]
    e_score = emergency_score[casualty['emergency'].lower()]
    pad_scores[pad] += s_score * e_score

# Print results
print("\nPriority Scores per Pad:")
for pad, score in pad_scores.items():
    print(f"{pad} -> {score}")
